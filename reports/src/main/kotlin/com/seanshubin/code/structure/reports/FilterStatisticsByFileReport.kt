...
class FilterStatisticsByFileReport(
    private val filterStats: FilterStats,
    private val categoryName: String
) : Report {
    override val reportName: String = "filter-statistics-$categoryName-by-file"
    ...
    override fun generate(baseReportDir: Path, validated: Validated): List<Command> {
        ...
        val fileToPatterns = matchedEvents
            ...
            .toSortedMap(compareBy { it.toDisplayString() })

        val tableRows = fileToPatterns.flatMap { (file, patterns) ->
            patterns.mapIndexed { index, pattern ->
                if (index == 0) {
                    FilePatternRow(file.toDisplayString(), pattern)
                } else {
        ...
        val htmlInsideBody = HtmlUtil.createTableWithText(
            ...
            caption = "Files Sorted by Name ($categoryName)"
        )

        val html = ReportHelper.wrapInTopLevelHtml(
            "By File: $categoryName",
            htmlInsideBody,
            listOf(Page.tableOfContents, Page.filterStatistics)
        )
        val path = reportDir.resolve("filter-statistics-$categoryName-by-file.html")
